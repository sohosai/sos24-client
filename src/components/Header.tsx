...
const HeaderNavigation: FC<{ isCommittee: boolean }> = ({ isCommittee }) => {
  const commonItemStyle = css({ display: "block", height: "100%", lineHeight: "100%" });
  return (<ul className={css({
    ...
    <li>
      <Link href={`${isCommittee ? "/committee" : ""}/dashboard`} className={commonItemStyle}>企画情報</Link>
    </li>
    <li>
      <Link href={`${isCommittee ? "/committee" : ""}/forms`} className={commonItemStyle}>申請</Link>
    </li>
    <li>
      <Link href={`${isCommittee ? "/committee" : ""}/news`} className={commonItemStyle}>お知らせ</Link>
    </li>
  </ul>);
};
...
const SwitchModeButton: FC<{ isCommitteeMode: boolean, showMobileMenu: boolean }> = ({
  ...
  <Link href={isCommitteeMode ? "/dashboard" : "/committee/dashboard"}>
    <button
      ...
      <Image src={showMobileMenu ? ModeSwitchWhite : ModeSwitch} alt="人のアイコンの周囲に矢印"
             className={css({
...
const MobileMenu: FC<{ isCommittee: boolean, isCommitteeMode: boolean, show: boolean, signOutFunc: () => void }> = ({
                                                                                                                      isCommittee,
                                                                                                                      isCommitteeMode,
                                                                                                                      show,
                                                                                                                      signOutFunc
                                                                                                                    }) => (
  <nav className={show ? css({
    display: "flex",
    justifyContent: "center",
    flexDir: "column",
    alignItems: "center",
    ...
    <ul className={css({
      ...
      <li className={MobileMenuItemStyle}><Link href={"./dashboard"}>企画情報</Link></li>
      <li className={MobileMenuItemStyle}><Link href={"./forms"}>申請一覧</Link></li>
      <li className={MobileMenuItemStyle}><Link href={"./news"}>お知らせ一覧</Link></li>
    </ul>
    {isCommittee && (
      <Link className={MobileMenuItemStyle}
            href={`${isCommitteeMode ? "" : "/committee"}/dashboard`}>{isCommitteeMode ? "一般" : "実委人"}ページへ</Link>
    )}
    <button className={css({ border: "2px solid", paddingX: 10, paddingY: 2 })} onClick={signOutFunc}>ログアウト
    </button>
  </nav>
);
...
export const Header: FC = () => {
  ...
  return (
    <header className={css({
      ...
      <Toaster />
      {showMobileMenu && (
        <MobileMenu isCommittee={["committee", "committee_operator", "administrator"].includes(userInfo?.role ?? "")}
                    show={showMobileMenu}
      ...
      <div
        ...
        {user ?
          (
            <button className={css({ display: "flex", justifyContent: "center", sm: { display: "none" } })}
                    onClick={() => setShowMobileMenu(e => !e)}>
              {showMobileMenu ? <Image src={CloseButton} alt={"バツボタン"} /> :
