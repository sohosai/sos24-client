  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}
      ...
      <div className={labelAndInputStyle}>
        <label htmlFor="name" className={css({ fontWeight: "bold" })}>名前</label>
      ...
      <div className={labelAndInputStyle}>
        <label htmlFor="kana_name" className={css({ fontWeight: "bold" })}>なまえ</label>
        <input type="text" id="kana_name" placeholder="かのうじごろう"
               className={cx(basicFormStyle())} {...register("kana_name", {
      ...
      <div className={labelAndInputStyle}>
        ...
        <input type="email" id="email" placeholder="xxxx@x.tsukuba.ac.jp"
               className={cx(basicFormStyle())} {...register("email", {
      ...
      <div className={labelAndInputStyle}>
        ...
        <input type="password" id="password"
               className={cx(basicFormStyle())} {...register("password", { required: "パスワードを入力してください" })} />
        {errors.password && <span className={basicErrorMessageStyle}>{errors.password.message}</span>}
      </div>
      <div className={labelAndInputStyle}>
        <label htmlFor="phone_number" className={css({ fontWeight: "bold" })}>電話番号</label>
        <input type="tel" id="phone_number"
               className={cx(basicFormStyle())} {...register("phone_number", { required: "電話番号を入力してください" })} />
        {errors.phone_number && <span className={basicErrorMessageStyle}>{errors.phone_number.message}</span>}
      </div>
      <div className={css({ display: "flex", alignItems: "center", gap: 3 })}>
        <input type="checkbox" id="agreement"
               className={checkboxFormStyle} {...register("agreement", { required: "利用規約に同意してください" })} />
        <label
